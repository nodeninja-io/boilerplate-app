...
var debug         = require('debug')('nodeninja');       // https://github.com/visionmedia/debug
...
module.exports.controller = function (app) {
  ...
  app.get('/api/socrata', function (req, res, next) {
    ...
    request.get({ url: 'https://opendata.socrata.com/resource/pssf-frzk.json?$order=issue_date DESC', timeout: 5000 }, function (err, response, body) {
      if (!err && response.statusCode === 200) {
        // Parse the data
  ...
  app.get('/api/twitter', passportConf.isAuthenticated, passportConf.isAuthorized, function (req, res, next) {
    ...
    var params = { q: 'Open Data', since_id: 24012619984051000, count: 10, result_type: 'popular' };
    var T = new Twit({
